...
###############################################################################################
{
    echo -e "   📦️     Installing Base Packages..." | lsd-print
      ...
      PACKAGES1=(
        base-devel hyprcursor hyprgraphics hypridle hyprlang hyprpaper nwg-look
...
###############################################################################################
{
    log_status "  🛠️   Applying base configurations..." | lsd-print
      STOW_DIRS=(
        ...
        "ghostty" "gtk-3.0" "gtk-4.0" "home" "htop" "hypr" "kitty" "nemo" "nvim"
        "nwg-dock-hyprland" "rofi" "nwg-look" "pacseek" "pomodorolm" "qt6ct" "scripts"
        "sddm" "settings" "systemd" "vim" "vlc" "wal" "waybar" "waypaper" "wlogout"
        "xdg-desktop-portal" "xsettingsd" "yazi" "znt" "oh-my-zsh"
        )
        cd ~/.dotfiles
...
###############################################################################################
{
    echo -e "   📦️    Installing Main Packages..." | lsd-print
      PACKAGES2=(
        amd-ucode ark aylurs-gtk-shell blueberry blueprint-compiler bluez bluez-utils
        bpytop brightnessctl btrfs-progs cliphist cmake cmatrix duf dunst efibootmgr
        eza fastfetch figlet fortune-mod fortune-mod-archlinux fzf ghostty go grim
        grimblast-git gst-plugin-pipewire gum htop intel-media-driver iwd kate konqueror
        kvantum libpulse libva-intel-driver lsd lua meson neovim-lspconfig
        neovim-web-devicons-git noto-fonts noto-fonts-emoji otf-fira-sans otf-font-awesome
        pavucontrol pinta pipewire pipewire-alsa pipewire-jack pipewire-pulse pomodorolm
        prettier python-hyprpy python-pillow python-vlc rofi-calc slurp smile starship timeshift
        timeshift-autosnap tmux vala vim vlc vlc-materia-skin-git vulkan-intel vulkan-radeon
        wget wireless_tools wireplumber wl-clipboard wl-clipboard-history-git  wlogout wtype
        xclip xdg-utils xf86-video-amdgpu xf86-video-ati  xf86-video-nouveau xf86-video-vmware
        xorg-server xorg-wayland xorg-xhost xorg-xinit xsettingsd yazi zig zoxide zram-generator
